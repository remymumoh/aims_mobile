...
class RfcReportsList extends StatefulWidget {
  final bool submitted;
  final bool rejectedByTechLead;
  final bool rejectedByRfc;
  const RfcReportsList(this.submitted, this.rejectedByTechLead, this.rejectedByRfc);
  @override
  State<StatefulWidget> createState() => _RfcReportsListState(submitted, rejectedByTechLead, rejectedByRfc);
}

class _RfcReportsListState extends State<RfcReportsList> {
  ...
  _RfcReportsListState(this.submitted, this.rejectedByTechLead, this.rejectedByRfc);
  ...
  void getClusters() async {
    ...
    this.clusters.clear();
    try {
      var response = rejectedByTechLead ? await http.get("$hostUrl/entry/rfc-tech-lead-rejected-reports?rfcId=$id")  // forms rejected by tech lead (above rfc)
          : rejectedByRfc ? await http.get("$hostUrl/entry/rfc-reviewed-reports?rfcId=$id&accepted=false")  // forms rfc rejected
          : submitted ? await http.get("$hostUrl/entry/rfc-reviewed-reports?rfcId=$id&accepted=true")   // forms rfc has accepted/submitted
          : await http.get("$hostUrl/entry/rfc-pending-reports?rfcId=$id");  // forms pending review by rfc
  ...
  Future<bool> performSyncToDb() async {
    ...
    var response = await http.get("$hostUrl/entry?rfcId=$id");
    log("getting surveys at: $hostUrl/entry?rfcId=$id");
  ...
  goToSummary(int index) async {
    bool result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) {
      return RfcReportSummary(this.template, this.clusters[index].form_name, this.clusters[index]);
    }));
  }

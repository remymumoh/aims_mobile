...
import 'package:aims_mobile/screens/forms/tech_lead/tech_lead_report_summary.dart';
...
class TechLeadReportsList extends StatefulWidget {
  final bool submitted;
  final bool rejectedByEthics;
  final bool rejectedByTechLead;

  const TechLeadReportsList(this.submitted, this.rejectedByEthics, this.rejectedByTechLead);

  @override
  State<StatefulWidget> createState() => _TechLeadReportsListState(submitted, rejectedByEthics, rejectedByTechLead);
}

class _TechLeadReportsListState extends State<TechLeadReportsList> {
  ...
  _TechLeadReportsListState(this.submitted, this.rejectedByEthics, this.rejectedByTechLead);
  ...
  void getClusters() async {
    ...
    this.clusters.clear();
    try {
      var response = rejectedByEthics ? await http.get("$hostUrl/entry/tech-lead-ethics-rejected-reports?techLeadId=$id")  // forms rejected by ethics lead/assistant (above tech lead)
          : rejectedByTechLead ? await http.get("$hostUrl/entry/tech-lead-reviewed-reports?techLeadId=$id&accepted=false")  // forms tech lead rejected
          : submitted ? await http.get("$hostUrl/entry/tech-lead-reviewed-reports?techLeadId=$id&accepted=true")   // forms tech lead has accepted/submitted
          : await http.get("$hostUrl/entry/tech-lead-pending-reports?techLeadId=$id");  // forms pending review by tech lead
  ...
  Future<bool> performSyncToDb() async {
    ...
    // get all reports with tech lead's id
    var response = await http.get("$hostUrl/entry?techLeadId=$id");
    // log("performSyncToDb url: http://10.0.2.2:8080/api/v1/entry?techLeadId=$id");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Technical Lead Review"),
      ),
      body: Container(
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  leading: CircleAvatar(
                    backgroundColor: getApprovalColor(this.clusters[position].techLeadAccepted == true ? 1 : this.clusters[position].techLeadAccepted == false ? 2 : 0),
                    child: getApprovalIcon(this.clusters[position].techLeadAccepted == true ? 1 : this.clusters[position].techLeadAccepted == false ? 2 : 0),
                  ),
                  subtitle: Text(
  ...
  goToSummary(int index) async {
    bool result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) {
      return TechLeadReportSummary(this.template, this.clusters[index].form_name, this.clusters[index]);
    }));

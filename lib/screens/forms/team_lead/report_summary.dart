...
class ReportSummary extends StatefulWidget {
  ReportSummary(this.template, this.formName, this.dbForm);
  final String formName;
  final DbForm dbForm;
  final FormTemplate template;

  @override
  State<StatefulWidget> createState() => _ReportSummaryState(template, formName, dbForm);
}
...
class _ReportSummaryState extends State<ReportSummary> {
  ...
  _ReportSummaryState(this.template, this.formName, this.dbForm);
  ...
  Widget generateSummary() {
    return ListView(
      ...
      children: <Widget>[
        ...
        ListTile(
          title: Text("Submission status: ${this.dbForm.leadAccepted == true ? "Submitted" : "Not submitted"}"),
        ),
        ExpansionPanelList(
        ...
        if (dbForm.rfcAccepted == false) Container(
          margin: EdgeInsets.all(15.0),
          child: Text("This report was rejected by the RFC", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 24.0), textAlign: TextAlign.center,),
        )
      ]
    );
  }
  ...
  void goToDetails() async {
    bool result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) {
      ...
      return ReportsView(this.template, this.dbForm);
    }));
  ...
  void submitComment() {
    ...
    if (leadFormComments.length == 0) {
      List<String> tempList = <String>[commentCtrl.text];
      var temp = jsonEncode(tempList);
      commentToStore = temp;
    } else {
      debugPrint("${jsonDecode(dbForm.leadComments)}");
      leadFormComments.add(commentCtrl.text);
      debugPrint("${leadFormComments.toString()}");
      commentToStore = jsonEncode(leadFormComments);
    }
  ...
  void deleteComment(int index) {
    ...
    leadFormComments.removeAt(index);
    debugPrint("comments after removal: ${jsonEncode(leadFormComments)}");

    updateComments(jsonEncode(leadFormComments), false);
  }
  ...
  void updateComments(String newComments, bool submitted) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (result != 0) {
        setState(() {
          dbForm = form;
          leadFormComments = jsonDecode(jsonDecode(response.body)["leadComments"]).cast<String>();
          _data = generateItems();
          generateSummary();
          commentCtrl.clear();
        });

...
import 'ethics_report_summary.dart';

class EthicsReportsList extends StatefulWidget {
  final bool submitted;
  final bool returnedByProjectLead;
  final bool rejectedByEthics;

  const EthicsReportsList(this.submitted, this.returnedByProjectLead, this.rejectedByEthics);
  @override
  State<StatefulWidget> createState() => _EthicsReportsListState(submitted, returnedByProjectLead, rejectedByEthics);
}

class _EthicsReportsListState extends State<EthicsReportsList> {
 ...
 FormsDatabase db = FormsDatabase.getInstance();
  ...
  int count = 0;
  int status = 1;
  final bool submitted, returnedByProjectLead, rejectedByEthics;
  String syncMessage = "Updated Successfully";
  bool syncInProgress = false;
  String hostUrl = "http://10.0.2.2:8080/api/v1";
  
  _EthicsReportsListState(this.submitted, this.returnedByProjectLead, this.rejectedByEthics);
  ...
  void getClusters() async {
    ...
    this.clusters.clear();
    try {
      var response = returnedByProjectLead ? await http.get("$hostUrl/entry/ethics-reports-from-project-lead?ethicsId=$id")  // forms returned by project lead (above ethics)
          : rejectedByEthics ? await http.get("$hostUrl/entry/ethics-reviewed-reports?ethicsId=$id&accepted=false")  // forms ethics rejected
          : submitted ? await http.get("$hostUrl/entry/ethics-reviewed-reports?ethicsId=$id&accepted=true")   // forms ethics has accepted/submitted
          : await http.get("$hostUrl/entry/ethics-pending-reports?ethicsId=$id");  // forms pending review by ethic
      // var response = await http.get("$hostUrl/entry?ethicsId=$id");
      // log(response.body);
  ...
  goToSummary(int index) async {
    bool result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) {
      return EthicsReportSummary(this.template, this.clusters[index].form_name, this.clusters[index]);
    }));

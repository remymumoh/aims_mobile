...
class _EthicsReportViewState extends State<EthicsReportsView> {
  ...
  int count = 0;
  bool saving = false;
  String username, role, techLead, rfc, projectLead, processType, satelliteLead;
  ...
  sendSubmission() async {
    ...
    if (approved) { // if approved, send to project lead
      var submission = {
        'submittedTo': projectLead,
    ...
    } else {  // else, send back one stage determined by process (LAB or FIELD)
      var submission = {
        'submitter': dbForm.updated_by ?? username,
        'date': DateTime.now(),
        'submissionId': Uuid().v4()
      };
      if (processType == "LAB") { // if this is a LAB process, send to satellite lead
        submission['submittedTo'] = satelliteLead;
      } else {
        submission['submittedTo'] = rfc;
      }
      QuerySnapshot deviceTokens = await fdb.collection('users').document(submission['submittedTo']).collection('tokens').getDocuments();
      if (deviceTokens.documents.length > 0) {
        await fdb.collection('submissions').document(deviceTokens.documents[0].data['token']).setData(submission);
  ...
  void getInfo() async {
    ...
    projectLead = records['projectLeadId'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("${role == "ROLE_ETHICS_LEAD" ? "Ethics Lead" : "Ethics Assistant"} Review Report"),
          ...
          Padding(
            ...
            child: Row(
              children: <Widget>[
                ...
                Text("${approved == true ?
                "This form has been accepted" : approved == false ?
                "This form has been rejected" : "This form has not been reviewed yet"}")
              ],
            ),
          ),
          getFormPanel(),
          Padding(
              ...
              child: Row(
                  children: <Widget>[
                    ...
                    Expanded(
                      child: RaisedButton(
                        // child: Text((dbForm.ethicsAccepted != false && dbForm.ethicsAssistantAccepted != false) ? "Reject" : "Undo Reject", style: TextStyle(color: Colors.white)),
  ...
  Future<void> approveOrReject(bool status) async {
    ...
    bool ethicsAccepted, ethicsReviewed, ethicsAssistantAccepted, ethicsAssistantReviewed;
    if (status == null) {
      ethicsAccepted = null;
      ethicsReviewed = null;
      ethicsAssistantAccepted = null;
      ethicsAssistantReviewed = null;
    } else if (status != null && role == "ROLE_ETHICS_LEAD") {
      ethicsReviewed = true;
      ethicsAccepted = status;
      ethicsAssistantReviewed = dbForm.reviewedByEthicsAssistant;
      ethicsAssistantAccepted = dbForm.ethicsAssistantAccepted;
    } else if (status != null && role == "ROLE_ETHICS_ASSISTANT") {
      ethicsReviewed = dbForm.reviewedByEthics;
      ethicsAccepted = dbForm.ethicsAccepted;
      ethicsAssistantReviewed = true;
      ethicsAssistantAccepted = status;
    }
    log("ethicsAccepted: $ethicsAccepted");
    log("ethicsReviewed: $ethicsReviewed");
    log("ethicsAssistantReviewed: $ethicsAssistantReviewed");
    log("ethicsAssistantAccepted: $ethicsAssistantAccepted");
    try {
      var body = jsonEncode({
        "id": dbForm.id,
        ...
        "submittedToTeamLead": dbForm.submittedToTeamLead,
        "ethicsId": dbForm.ethicsId,
        "reviewedByEthics": ethicsReviewed,
        "ethicsAccepted": ethicsAccepted,
        "ethicsComments": dbForm.ethicsComments,
        "ethicsAssistantId": dbForm.ethicsAssistantId,
        "reviewedByEthicsAssistant": ethicsAssistantReviewed,
        "ethicsAssistantAccepted": ethicsAssistantAccepted,
        "ethicsAssistantComments": dbForm.ethicsAssistantComments,
      ...
      // log("ethics assist accept ${newForm.ethicsAssistantAccepted}");
  ...
  void _showSubmitConfirmDialog(bool accept) {
    if ((dbForm.ethicsAccepted == true || dbForm.ethicsAssistantAccepted == true) && accept == true) {
      AlertDialog dialog = AlertDialog(
        content: Text('This form has already been shared with the project director. Would you like to share this updated form?'),
        actions: <Widget>[
          FlatButton(
            child: Text('No'),
            onPressed: () {
    ...
    } else if ((dbForm.ethicsAccepted == false || dbForm.ethicsAssistantAccepted == false) && accept == false) {
      AlertDialog dialog = AlertDialog(
        // content: Text('This report has been rejected. Would you like to undo this rejection?'),
        content: Text('This report has already been rejected'),
        actions: <Widget>[
          FlatButton(
            child: Text('OK'),
            onPressed: () {
      ...
      AlertDialog dialog = AlertDialog(
        content: Text('Are you sure you want to ${accept ? "share this form with the project lead?" : "reject this form and send it back to the technical lead?"}'),
        actions: <Widget>[
          FlatButton(
            child: Text('No'),
            onPressed: () {

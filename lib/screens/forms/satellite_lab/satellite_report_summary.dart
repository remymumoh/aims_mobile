...
import 'package:aims_mobile/screens/forms/satellite_lab/satellite_reports_view.dart';
...
class SatelliteReportSummary extends StatefulWidget {
  SatelliteReportSummary(this.template, this.formName, this.dbForm);
  final String formName;
  final DbForm dbForm;
  final FormTemplate template;

  @override
  State<StatefulWidget> createState() => _SatelliteReportSummaryState(template, formName, dbForm);
}
...
class _SatelliteReportSummaryState extends State<SatelliteReportSummary> {
  String formName;
  DbForm dbForm;
  ...
  _SatelliteReportSummaryState(this.template, this.formName, this.dbForm);
  ...
  void initState() {
    ...
    techFormComments = dbForm.processType == "FIELD" ? ((dbForm.techLeadComments != null) ? jsonDecode(dbForm.techLeadComments).cast<String>() : <String>[]) : [];
    satelliteComments = dbForm.processType == "LAB" ? ((dbForm.satelliteLeadComments != null) ? jsonDecode(dbForm.satelliteLeadComments).cast<String>() : <String>[]) : [];
  ...
  List<Item> generateItems() {
    List<String> titles = [
      "Team Lead Comments",
      "RFC Comments",
      if (dbForm.processType == "FIELD") "Technical Lead Comments",
      if (dbForm.processType == "LAB") "Satellite Lab Lead Comments",
      "Ethics Lead Comments",
      "Ethics Assistant Comments",
      "Project Lead Comments"
    ];
    List<dynamic> panels = [
      (leadFormComments.length > 0) ? getLeadCommentList() : Padding(child: Text("No comments from team lead"), padding: EdgeInsets.all(15.0)),
  ...
  Widget generateSummary() {
    return ListView(
        ...
        children: <Widget>[
          ...
          ListTile(
            title: Text("Submission status: ${this.dbForm.satelliteLeadAccepted == true ? "Submitted" : "Not submitted"}"),
          ),
          ExpansionPanelList(
          ...
          if (dbForm.ethicsAssistantAccepted == false && dbForm.ethicsAccepted != false) Container(
            margin: EdgeInsets.all(15.0),
          ...
          if (dbForm.satelliteLeadAccepted == false) Container(
            margin: EdgeInsets.all(15.0),
  ...
  void goToDetails() async {
    bool result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) {
      return SatelliteReportsView(this.template, this.dbForm);
    }));
  ...
  void submitComment() {
    ...
    if (rfcFormComments.length == 0) {
      List<String> tempList = <String>[commentCtrl.text];
      var temp = jsonEncode(tempList);
      commentToStore = temp;
    } else {
      debugPrint("${jsonDecode(dbForm.rfcComments)}");
      rfcFormComments.add(commentCtrl.text);
      debugPrint("${rfcFormComments.toString()}");
      commentToStore = jsonEncode(rfcFormComments);
    }
  ...
  void deleteComment(int index) {
    ...
    rfcFormComments.removeAt(index);
    debugPrint("comments after removal: ${jsonEncode(rfcFormComments)}");

    updateComments(jsonEncode(rfcFormComments), false);
  }
  ...
  void updateComments(String newComments, bool submitted) async {
    var body = jsonEncode({
      ...
      "satelliteLeadComments": newComments,
    ...
    if (response.statusCode == 200) {
      ...
      if (result != 0) {
        setState(() {
          dbForm = form;
          rfcFormComments = jsonDecode(jsonDecode(response.body)["rfcComments"]).cast<String>();
          _data = generateItems();
          generateSummary();
          commentCtrl.clear();
        });
      ...
      } else {  // if comment gets saved to PHIA server but not local db
        final snackBar = SnackBar(
    ...
    } else {  // if data does not get stored on PHIA properly, show snackbar with fail message
      final snackBar = SnackBar(
        content: Text('Sorry, your comment could not be ${submitted ? "submitted" : "deleted"}. Please try again later.'),
        action: SnackBarAction(
          label: 'Done',

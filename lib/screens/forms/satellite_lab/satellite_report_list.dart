...
import 'package:aims_mobile/screens/forms/satellite_lab/satellite_report_summary.dart';
...
class SatelliteReportsList extends StatefulWidget {
  final bool submitted;
  final bool rejectedByTechLead;
  final bool rejectedByRfc;
  const SatelliteReportsList(this.submitted, this.rejectedByTechLead, this.rejectedByRfc);
  @override
  State<StatefulWidget> createState() => _SatelliteReportsListState(submitted, rejectedByTechLead, rejectedByRfc);
}

class _SatelliteReportsListState extends State<SatelliteReportsList> {
  ...
  bool loading = false;
  int count = 0;
  final bool submitted, rejectedByTechLead, rejectedByRfc;
  String syncMessage = "Updated Successfully";
  bool syncInProgress = false;
  String hostUrl = "http://10.0.2.2:8080/api/v1";
  // String hostUrl = "http://10.0.2.2:8080/api/v1";

  _SatelliteReportsListState(this.submitted, this.rejectedByTechLead, this.rejectedByRfc);
  ...
  void getClusters() async {
    ...
    this.clusters.clear();
    try {
      var response = rejectedByTechLead ? await http.get("$hostUrl/entry/satellite-lead-ethics-lead-rejected-reports?satelliteId=$id")  // forms rejected by ethics (above satellite lab)
          : rejectedByRfc ? await http.get("$hostUrl/entry/satellite-lead-reviewed-reports?satelliteId=$id&accepted=false")  // forms satellite rejected
          : submitted ? await http.get("$hostUrl/entry/satellite-lead-reviewed-reports?satelliteId=$id&accepted=true")   // forms satellite has accepted/submitted
          : await http.get("$hostUrl/entry/satellite-lead-pending-reports?satelliteId=$id");  // forms pending review by satellite
      // log(response.body);
  ...
  Future<bool> performSyncToDb() async {
    ...
    // get all submitted reports with current lead's id
    var response = await http.get("$hostUrl/entry?satelliteId=$id");
    log("getting surveys at: $hostUrl/entry?satelliteId=$id");
    // log("${jsonDecode(response.body)}");
  ...
  Icon getApprovalIcon(int approve) {
    return (rejectedByRfc || rejectedByTechLead)? Icon(
        Icons.dangerous,
        color: Colors.white
    ) : approve == 1 ? Icon(
      Icons.check,
  ...
  Color getApprovalColor(int status) {
    return (rejectedByRfc || rejectedByTechLead) ? Colors.red : status == 1 ? Colors.green : Colors.grey;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Satellite Lab Lead Review"),
      ),
      body: Container(
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  leading: CircleAvatar(
                    backgroundColor: getApprovalColor(this.clusters[position].rfcAccepted == true ? 1 : this.clusters[position].rfcAccepted == false ? 2 : 0),
                    child: getApprovalIcon(this.clusters[position].rfcAccepted == true ? 1 : this.clusters[position].rfcAccepted == false ? 2 : 0),
                  ),
                  subtitle: Text(
  ...
  goToSummary(int index) async {
    bool result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) {
      return SatelliteReportSummary(this.template, this.clusters[index].form_name, this.clusters[index]);
    }));

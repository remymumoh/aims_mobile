...
import 'project_lead_reports_view.dart';

class ProjectLeadReportSummary extends StatefulWidget {
  ProjectLeadReportSummary(this.template, this.formName, this.dbForm);
  ...
  State<StatefulWidget> createState() =>
      _ProjectLeadReportSummaryState(template, formName, dbForm);
}
...
class _ProjectLeadReportSummaryState extends State<ProjectLeadReportSummary> {
  ...
  _ProjectLeadReportSummaryState(this.template, this.formName, this.dbForm);

  @override
  void initState() {
    processType = dbForm.processType ?? "FIELD";
    rfcFormComments = (dbForm.rfcComments != null)
        ? jsonDecode(dbForm.rfcComments).cast<String>()
        : <String>[];
    leadFormComments = (dbForm.leadComments != null)
        ? jsonDecode(dbForm.leadComments).cast<String>()
        : <String>[];
    ethicsFormComments = (dbForm.ethicsComments != null)
        ? jsonDecode(dbForm.ethicsComments).cast<String>()
        : <String>[];
    if (processType == "FIELD") {
      techFormComments = (dbForm.techLeadComments != null)
          ? jsonDecode(dbForm.techLeadComments).cast<String>()
          : <String>[];
    } else if (processType == "LAB") {
      satelliteComments = (dbForm.satelliteLeadComments != null)
          ? jsonDecode(dbForm.satelliteLeadComments).cast<String>()
          : <String>[];
    }
    ethicsAssistantComments = (dbForm.ethicsAssistantComments != null)
        ? jsonDecode(dbForm.ethicsAssistantComments).cast<String>()
        : <String>[];
    projectLeadComments = (dbForm.projectLeadComments != null)
        ? jsonDecode(dbForm.projectLeadComments).cast<String>()
        : <String>[];
    _data = generateItems();
  ...
  List<Item> generateItems() {
    ...
    List<dynamic> panels = [
      (leadFormComments.length > 0)
          ? getLeadCommentList()
          : Padding(
              child: Text("No comments from lead"),
              padding: EdgeInsets.all(15.0)),
      (rfcFormComments.length > 0)
          ? getRfcCommentList()
          : Padding(
              child: Text("No comments from RFC"),
              padding: EdgeInsets.all(15.0)),
      if (processType == "FIELD")
        (techFormComments.length > 0)
            ? getTechLeadCommentList()
            : Padding(
                child: Text("No comments from technical lead"),
                padding: EdgeInsets.all(15.0)),
      if (processType == "LAB")
        (satelliteComments.length > 0)
            ? getSatelliteLeadCommentList()
            : Padding(
                child: Text("No comments from technical lead"),
                padding: EdgeInsets.all(15.0)),
      (ethicsFormComments.length > 0)
          ? getEthicsCommentList()
          : Padding(
              child: Text("No comments from ethics lead"),
              padding: EdgeInsets.all(15.0),
            ),
      (ethicsAssistantComments.length > 0)
          ? getEthicsAssistantCommentList()
          : Padding(
              child: Text("No comments from ethics assistant"),
              padding: EdgeInsets.all(15.0),
            ),
      (projectLeadComments.length > 0)
          ? getProjectLeadCommentList()
          : Padding(
              child: Text("No comments from project lead"),
              padding: EdgeInsets.all(15.0)),
    ];
      ...
      body: SingleChildScrollView(
        child: Column(children: <Widget>[
          Container(
            ...
            child: Text("Summary for $formName",
  ...
  Widget generateSummary() {
    return ListView(shrinkWrap: true, children: <Widget>[
      ...
      ListTile(
        title: Text(
            "Updated on ${DateFormat.yMMMd("en_US").format(DateTime.parse(this.dbForm.date_updated))} by ${dbForm.updated_by}"),
      ),
      ListTile(
        title: Text(
            "Accepted: ${this.dbForm.projectLeadReturned == true ? "Yes" : "No"}"),
      ),
      ExpansionPanelList(
          ...
          children: [
            ExpansionPanel(
                ...
                headerBuilder: (BuildContext context, bool isExpanded) {
                  return ListTile(title: Text(_data[0].headerValue));
                },
            ...
            ExpansionPanel(
                canTapOnHeader: true,
                headerBuilder: (BuildContext context, bool isExpanded) {
                  return ListTile(title: Text(_data[1].headerValue));
                },
            ...
            ExpansionPanel(
                canTapOnHeader: true,
                headerBuilder: (BuildContext context, bool isExpanded) {
                  return ListTile(title: Text(_data[2].headerValue));
                },
            ...
            ExpansionPanel(
                canTapOnHeader: true,
                headerBuilder: (BuildContext context, bool isExpanded) {
                  return ListTile(title: Text(_data[3].headerValue));
                },
            ...
            ExpansionPanel(
                canTapOnHeader: true,
                headerBuilder: (BuildContext context, bool isExpanded) {
                  return ListTile(title: Text(_data[4].headerValue));
                },
            ...
            ExpansionPanel(
                canTapOnHeader: true,
                headerBuilder: (BuildContext context, bool isExpanded) {
                  return ListTile(title: Text(_data[5].headerValue));
                },
                isExpanded: _data[5].isExpanded,
                body: _data[5].expandedValue),
          ]),
      if (dbForm.projectLeadReturned == false)
        Container(
          margin: EdgeInsets.all(15.0),
          child: Text(
            "This report was rejected by you",
  ...
  void goToDetails() async {
    bool result = await Navigator.push(context,
      ...
      return ProjectLeadReportsView(this.template, this.dbForm);
    }));
    if (result == true) {
  ...
  void submitComment() {
    ...
    if (projectLeadComments.length == 0) {
      List<String> tempList = <String>[commentCtrl.text];
      var temp = jsonEncode(tempList);
      commentToStore = temp;
    } else {
      debugPrint("${jsonDecode(dbForm.projectLeadComments)}");
      projectLeadComments.add(commentCtrl.text);
      debugPrint("${projectLeadComments.toString()}");
      commentToStore = jsonEncode(projectLeadComments);
    }
  ...
  void deleteComment(int index) {
    ...
    projectLeadComments.removeAt(index);
    debugPrint("comments after removal: ${jsonEncode(projectLeadComments)}");
  ...
  void updateComments(String newComments, bool submitted) async {
    var body = jsonEncode({
      ...
      "projectLeadComments": newComments,
    });
    var response = await http.put("$hostUrl/entry/${dbForm.id}",
    ...
    if (response.statusCode == 200) {
      ...
      if (result != 0) {
        ...
        setState(() {
          projectLeadComments =
              jsonDecode(jsonDecode(response.body)["ethicsComments"])
                  .cast<String>();
          log("formComments is now: ${projectLeadComments.toString()}");
          _data = generateItems();
          commentCtrl.clear();
        });
        final snackBar = SnackBar(
          content:
              Text('Your comment was ${submitted ? "submitted" : "deleted"}.'),
        ...
        final snackBar = SnackBar(
          content: Text(
              'Sorry, your comment could not be ${submitted ? "submitted" : "deleted"}. Please try again later.'),
          action: SnackBarAction(
            label: 'Done',
            onPressed: () {
              ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ...
      itemBuilder: (BuildContext context, int position) {
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text(dbForm.projectLeadId != null
                ? dbForm.projectLeadId
                : 'Project ID'),
            trailing: GestureDetector(
              child: Icon(Icons.delete, color: Colors.grey),
              onTap: () {
